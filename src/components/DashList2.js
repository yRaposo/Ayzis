...
export default function DashList2() {
    ...
    const fetchData = useCallback(async () => {
        setIsSearching(true);
        const data = await getSomaProdutosVendidos();
        setSomaQtdVendas(data || {});
    ...
    const allMonths = Array.from(
        new Set(
            Object.values(somaQtdVendas)
                .flatMap(prodData =>
                    Object.keys(prodData)
                        .filter(key => key.endsWith("_qtd"))
                        .map(key => key.replace("_qtd", ""))
                )
        )
    ).sort();

    // Filtra produtos pelo SKU digitado
    const filteredEntries = Object.entries(somaQtdVendas).filter(([produtoId]) =>
    ...
    return (
        <div className="flex flex-col items-center align-middle w-full">
            <h1 className="justify-center items-center text-4xl font-bold text-center">Soma de vendas mensais por produto</h1>
